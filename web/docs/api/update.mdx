...
```bash
curl "https://sheetmetal.io/api/v1/YOUR_METAL_KEY/TAB/COLUMN_NAME/COLUMN_VALUE" \
  -X PATCH \
...
```bash
curl "https://sheetmetal.io/api/v1/YOUR_METAL_KEY/Employees/gender/male" \
  -X PATCH \
  -H "Content-Type: application/json" \
  -d '{ "gender": "not hotdog" }'
```
...
```bash
curl "https://sheetmetal.io/api/v1/YOUR_METAL_KEY/Employees/id/1" \
  -X PATCH \
  -H "Content-Type: application/json" \
  -d '{ "name": "Erlich Bachman", "gender": "male", "status": "terminated" }'
```
